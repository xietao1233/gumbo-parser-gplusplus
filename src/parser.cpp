...
#define TAG_VALUE (1 << GUMBO_NAMESPACE_HTML)
#define TAG_SVG_VALUE (1 << GUMBO_NAMESPACE_SVG)
#define TAG_MATHML_VALUE (1 << GUMBO_NAMESPACE_MATHML)

#define TAG(tag) [GUMBO_TAG_##tag] = TAG_VALUE
#define TAG_SVG(tag) [GUMBO_TAG_##tag] = TAG_SVG_VALUE
#define TAG_MATHML(tag) [GUMBO_TAG_##tag] = TAG_MATHML_VALUE
...
static const GumboNode kActiveFormattingScopeMarker={GUMBO_NODE_DOCUMENT,0,0,GUMBO_INSERTION_NORMAL,0};
...
static void parser_state_init(GumboParser* parser) {
  GumboParserState* parser_state =
      (GumboParserState*)gumbo_parser_allocate(parser, sizeof(GumboParserState));
...
static GumboInsertionMode get_appropriate_insertion_mode(
  ...
  const GumboNode* node = (GumboNode*)open_elements->data[index];
  ...
  switch (node->v.element.tag) {
    case GUMBO_TAG_SELECT: {
      ...
      for (int i = index; i > 0; --i) {
        const GumboNode* ancestor = (GumboNode*)open_elements->data[i];
...
static bool tag_in_01(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_HEAD]=TAG_VALUE;
  temp[GUMBO_TAG_BODY]=TAG_VALUE;
  temp[GUMBO_TAG_HTML]=TAG_VALUE;
  temp[GUMBO_TAG_BR]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_02(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_BASE]=TAG_VALUE;
  temp[GUMBO_TAG_BASEFONT]=TAG_VALUE;
  temp[GUMBO_TAG_BGSOUND]=TAG_VALUE;
  temp[GUMBO_TAG_MENUITEM]=TAG_VALUE;
  temp[GUMBO_TAG_LINK]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_03(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_NOFRAMES]=TAG_VALUE;
  temp[GUMBO_TAG_STYLE]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_04(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool tag_in_05(
  ...
  temp[GUMBO_TAG_BGSOUND]=TAG_VALUE;
  temp[GUMBO_TAG_LINK]=TAG_VALUE;
  temp[GUMBO_TAG_META]=TAG_VALUE;
  temp[GUMBO_TAG_NOFRAMES]=TAG_VALUE;
  temp[GUMBO_TAG_STYLE]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_06(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_HEAD]=TAG_VALUE;
  temp[GUMBO_TAG_NOSCRIPT]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_07(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
  ...
  temp[GUMBO_TAG_META]=TAG_VALUE;
  temp[GUMBO_TAG_NOFRAMES]=TAG_VALUE;
  temp[GUMBO_TAG_SCRIPT]=TAG_VALUE;
  temp[GUMBO_TAG_STYLE]=TAG_VALUE;
  temp[GUMBO_TAG_TEMPLATE]=TAG_VALUE;
  temp[GUMBO_TAG_TITLE]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_08(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool tag_in_0A(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_ADDRESS]=TAG_VALUE;
  temp[GUMBO_TAG_ARTICLE]=TAG_VALUE;
  temp[GUMBO_TAG_ASIDE]=TAG_VALUE;
  temp[GUMBO_TAG_BLOCKQUOTE]=TAG_VALUE;
  temp[GUMBO_TAG_CENTER]=TAG_VALUE;
  temp[GUMBO_TAG_DETAILS]=TAG_VALUE;
  temp[GUMBO_TAG_DIR]=TAG_VALUE;
  temp[GUMBO_TAG_DIV]=TAG_VALUE;
  temp[GUMBO_TAG_DL]=TAG_VALUE;
  temp[GUMBO_TAG_FIELDSET]=TAG_VALUE;
  temp[GUMBO_TAG_FIGCAPTION]=TAG_VALUE;
  temp[GUMBO_TAG_FIGURE]=TAG_VALUE;
  temp[GUMBO_TAG_FOOTER]=TAG_VALUE;
  temp[GUMBO_TAG_HEADER]=TAG_VALUE;
  temp[GUMBO_TAG_HGROUP]=TAG_VALUE;
  temp[GUMBO_TAG_MENU]=TAG_VALUE;
  temp[GUMBO_TAG_MAIN]=TAG_VALUE;
  temp[GUMBO_TAG_NAV]=TAG_VALUE;
  temp[GUMBO_TAG_OL]=TAG_VALUE;
  temp[GUMBO_TAG_P]=TAG_VALUE;
  temp[GUMBO_TAG_SECTION]=TAG_VALUE;
  temp[GUMBO_TAG_SUMMARY]=TAG_VALUE;
  temp[GUMBO_TAG_UL]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_0B(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_H1]=TAG_VALUE;
  temp[GUMBO_TAG_H2]=TAG_VALUE;
  temp[GUMBO_TAG_H3]=TAG_VALUE;
  temp[GUMBO_TAG_H4]=TAG_VALUE;
  temp[GUMBO_TAG_H5]=TAG_VALUE;
  temp[GUMBO_TAG_H6]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_0C(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_PRE]=TAG_VALUE;
  temp[GUMBO_TAG_LISTING]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_0D(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_DD]=TAG_VALUE;
  temp[GUMBO_TAG_DT]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_0E(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
  ...
  temp[GUMBO_TAG_ASIDE]=TAG_VALUE;
  temp[GUMBO_TAG_BLOCKQUOTE]=TAG_VALUE;  
  temp[GUMBO_TAG_BUTTON]=TAG_VALUE;
  temp[GUMBO_TAG_CENTER]=TAG_VALUE;
  temp[GUMBO_TAG_DETAILS]=TAG_VALUE;
  temp[GUMBO_TAG_DIR]=TAG_VALUE;
  temp[GUMBO_TAG_DIV]=TAG_VALUE;
...
static bool tag_in_0F(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_B]=TAG_VALUE;
  temp[GUMBO_TAG_BIG]=TAG_VALUE;
  temp[GUMBO_TAG_CODE]=TAG_VALUE;
  temp[GUMBO_TAG_EM]=TAG_VALUE;
  temp[GUMBO_TAG_FONT]=TAG_VALUE;
  temp[GUMBO_TAG_I]=TAG_VALUE;
  temp[GUMBO_TAG_S]=TAG_VALUE;
  temp[GUMBO_TAG_SMALL]=TAG_VALUE;
  temp[GUMBO_TAG_STRIKE]=TAG_VALUE;
  temp[GUMBO_TAG_STRONG]=TAG_VALUE;
  temp[GUMBO_TAG_TT]=TAG_VALUE;
  temp[GUMBO_TAG_U]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_10(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_A]=TAG_VALUE;
  temp[GUMBO_TAG_B]=TAG_VALUE;
  temp[GUMBO_TAG_BIG]=TAG_VALUE;
  temp[GUMBO_TAG_CODE]=TAG_VALUE;
  temp[GUMBO_TAG_EM]=TAG_VALUE;
...
static bool tag_in_11(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_APPLET]=TAG_VALUE;
  temp[GUMBO_TAG_MARQUEE]=TAG_VALUE;
  temp[GUMBO_TAG_OBJECT]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_12(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_AREA]=TAG_VALUE;
  temp[GUMBO_TAG_BR]=TAG_VALUE;
  temp[GUMBO_TAG_EMBED]=TAG_VALUE;
  temp[GUMBO_TAG_IMG]=TAG_VALUE;
  temp[GUMBO_TAG_IMAGE]=TAG_VALUE;
  temp[GUMBO_TAG_KEYGEN]=TAG_VALUE;
  temp[GUMBO_TAG_WBR]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_13(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_PARAM]=TAG_VALUE;
  temp[GUMBO_TAG_SOURCE]=TAG_VALUE;
  temp[GUMBO_TAG_TRACK]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_14(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_OPTION]=TAG_VALUE;
  temp[GUMBO_TAG_OPTGROUP]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_15(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_RB]=TAG_VALUE;
  temp[GUMBO_TAG_RP]=TAG_VALUE;
  temp[GUMBO_TAG_RT]=TAG_VALUE;
  temp[GUMBO_TAG_RTC]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_16(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool tag_in_17(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_CAPTION]=TAG_VALUE;
  temp[GUMBO_TAG_COL]=TAG_VALUE;
  temp[GUMBO_TAG_COLGROUP]=TAG_VALUE;
  temp[GUMBO_TAG_FRAME]=TAG_VALUE;
  temp[GUMBO_TAG_HEAD]=TAG_VALUE;
  temp[GUMBO_TAG_TBODY]=TAG_VALUE;
  temp[GUMBO_TAG_TD]=TAG_VALUE;
  temp[GUMBO_TAG_TFOOT]=TAG_VALUE;
  temp[GUMBO_TAG_TH]=TAG_VALUE;
  temp[GUMBO_TAG_THEAD]=TAG_VALUE;
  temp[GUMBO_TAG_TR]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_18(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool tag_in_26(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_TABLE]=TAG_VALUE;
  temp[GUMBO_TAG_TBODY]=TAG_VALUE;
  temp[GUMBO_TAG_TFOOT]=TAG_VALUE;
  temp[GUMBO_TAG_THEAD]=TAG_VALUE;
  temp[GUMBO_TAG_TR]=TAG_VALUE;
...
static bool tag_in_27(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_INPUT]=TAG_VALUE;
  temp[GUMBO_TAG_KEYGEN]=TAG_VALUE;
  temp[GUMBO_TAG_TEXTAREA]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_28(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool tag_in_2B(
  ...
  temp[GUMBO_TAG_H6]=TAG_VALUE;
  temp[GUMBO_TAG_HEAD]=TAG_VALUE;
  temp[GUMBO_TAG_HR]=TAG_VALUE;
  temp[GUMBO_TAG_I]=TAG_VALUE;
  temp[GUMBO_TAG_IMG]=TAG_VALUE;
  temp[GUMBO_TAG_LI]=TAG_VALUE;
  temp[GUMBO_TAG_LISTING]=TAG_VALUE;
  temp[GUMBO_TAG_MENU]=TAG_VALUE;
  temp[GUMBO_TAG_META]=TAG_VALUE;
  temp[GUMBO_TAG_NOBR]=TAG_VALUE;
  ...
  temp[GUMBO_TAG_P]=TAG_VALUE;
  temp[GUMBO_TAG_PRE]=TAG_VALUE;
  temp[GUMBO_TAG_RUBY]=TAG_VALUE;
  temp[GUMBO_TAG_S]=TAG_VALUE;
  temp[GUMBO_TAG_SMALL]=TAG_VALUE;
  temp[GUMBO_TAG_SPAN]=TAG_VALUE;
  temp[GUMBO_TAG_STRONG]=TAG_VALUE;
  temp[GUMBO_TAG_STRIKE]=TAG_VALUE;
  temp[GUMBO_TAG_SUB]=TAG_VALUE;
  temp[GUMBO_TAG_SUP]=TAG_VALUE;
  temp[GUMBO_TAG_TABLE]=TAG_VALUE;
  temp[GUMBO_TAG_TT]=TAG_VALUE;
  temp[GUMBO_TAG_U]=TAG_VALUE;
  temp[GUMBO_TAG_UL]=TAG_VALUE;
  temp[GUMBO_TAG_VAR]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}

static bool tag_in_2C(
    const GumboToken* token, bool is_start) {
  char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_MGLYPH]=TAG_VALUE;
  temp[GUMBO_TAG_MALIGNMARK]=TAG_VALUE;
  return tag_in(token,is_start,temp);
}
...
static bool node_tag_in_set_01(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };

  temp[GUMBO_TAG_HTML]=TAG_VALUE;
  temp[GUMBO_TAG_TR]=TAG_VALUE;
  temp[GUMBO_TAG_TEMPLATE]=TAG_VALUE;
  return node_tag_in_set(node,temp);
}

static bool node_tag_in_set_02(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_03(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_04(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
  ...
  temp[GUMBO_TAG_OBJECT]=TAG_VALUE;
  temp[GUMBO_TAG_TEMPLATE]=TAG_VALUE;
  temp[GUMBO_TAG_MI]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_MO]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_MN]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_MS]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_MTEXT]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_ANNOTATION_XML]=TAG_MATHML_VALUE;
  temp[GUMBO_TAG_FOREIGNOBJECT]=TAG_SVG_VALUE;
  temp[GUMBO_TAG_DESC]=TAG_SVG_VALUE;
  temp[GUMBO_TAG_TITLE]=TAG_SVG_VALUE;
  return node_tag_in_set(node,temp);
}

static bool node_tag_in_set_05(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_06(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_07(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_08(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_09(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_0A(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
static bool node_tag_in_set_0B(const GumboNode* node) {
	char temp[GUMBO_TAG_LAST] = { 0 };
...
InsertionLocation get_appropriate_insertion_location(
  ...
  if (!parser->_parser_state->_foster_parent_insertions ||
      !node_tag_in_set(retval.target, temp)) {
    return retval;
  }
  ...
  for (unsigned int i = 0; i < open_elements->length; ++i) {
    if (node_html_tag_is((GumboNode*)open_elements->data[i], GUMBO_TAG_TEMPLATE)) {
      last_template_index = i;
    }
    if (node_html_tag_is((GumboNode*)open_elements->data[i], GUMBO_TAG_TABLE)) {
      last_table_index = i;
    }
  }
  if (last_template_index != -1 &&
      (last_table_index == -1 || last_template_index > last_table_index)) {
    retval.target = (GumboNode*)open_elements->data[last_template_index];
    return retval;
  }
  if (last_table_index == -1) {
    retval.target = (GumboNode*)open_elements->data[0];
    return retval;
  }
  GumboNode* last_table = (GumboNode*)open_elements->data[last_table_index];
  ...
  retval.target = (GumboNode*)open_elements->data[last_table_index - 1];
  return retval;
}
...
static GumboNode* pop_current_node(GumboParser* parser) {
  ...
  if (state->_open_elements.length > 0) {
    assert(node_html_tag_is((GumboNode*)state->_open_elements.data[0], GUMBO_TAG_HTML));
  ...
  if ((state->_current_token->type != GUMBO_TOKEN_END_TAG ||
    ...
    current_node->parse_flags = (GumboParseFlags)(current_node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG);
  }
...
static void clear_stack_to_table_row_context(GumboParser* parser) {
  while (!node_tag_in_set_01(get_current_node(parser))) {
...
static void clear_stack_to_table_context(GumboParser* parser) {
  while (!node_tag_in_set_02(get_current_node(parser)
             )) {
    pop_current_node(parser);
  }
}
...
void clear_stack_to_table_body_context(GumboParser* parser) {
  while (!node_tag_in_set_03(get_current_node(parser)
  )) {
    pop_current_node(parser);
  }
}
...
static GumboNode* insert_element_of_tag_type(
  ...
  element->parse_flags = (GumboParseFlags)(element->parse_flags | (GUMBO_INSERTION_BY_PARSER | reason));
  insert_element(parser, element, false);
...
GumboNode* clone_node(
  ...
  *new_node = *node;
  ...
  new_node->parse_flags = (GumboParseFlags)(new_node->parse_flags & ~GUMBO_INSERTION_IMPLICIT_END_TAG);
  new_node->parse_flags = (GumboParseFlags)(new_node->parse_flags | (reason | GUMBO_INSERTION_BY_PARSER));
...
static void reconstruct_active_formatting_elements(GumboParser* parser) {
  ...
  GumboNode* element = (GumboNode*)elements->data[i];
...
static bool has_an_element_in_scope(GumboParser* parser, GumboTag tag) {
	...
	temp[GUMBO_TAG_DESC] = TAG_SVG_VALUE;
	temp[GUMBO_TAG_TITLE] = TAG_SVG_VALUE;
	return has_an_element_in_specific_scope(parser, 1, &tag, false, temp);
...
static bool has_an_element_in_scope_with_tagname(
	...
	temp[GUMBO_TAG_DESC] = TAG_SVG_VALUE;
	temp[GUMBO_TAG_TITLE] = TAG_SVG_VALUE;
	return has_an_element_in_specific_scope(parser, expected_len, expected, false, temp);
...
static void generate_implied_end_tags(GumboParser* parser, GumboTag exception) {
  for (; node_tag_in_set_05(get_current_node(parser)) &&
...
static void generate_all_implied_end_tags_thoroughly(GumboParser* parser) {
  for (; node_tag_in_set_06(get_current_node(parser));
      pop_current_node(parser))
    ;
}
...
static bool is_special_node(const GumboNode* node) {
  ...
  temp[GUMBO_TAG_FIGURE] = TAG_VALUE;
  temp[GUMBO_TAG_FOOTER] = TAG_VALUE;
  temp[GUMBO_TAG_FORM] = TAG_VALUE;
  temp[GUMBO_TAG_FRAME] = TAG_VALUE;
  temp[GUMBO_TAG_FRAMESET] = TAG_VALUE;
  temp[GUMBO_TAG_H1] = TAG_VALUE;
  temp[GUMBO_TAG_H2] = TAG_VALUE;
  temp[GUMBO_TAG_H3] = TAG_VALUE;
  temp[GUMBO_TAG_H4] = TAG_VALUE;
  ...
  temp[GUMBO_TAG_HR] = TAG_VALUE;
  temp[GUMBO_TAG_HTML] = TAG_VALUE;
  temp[GUMBO_TAG_IFRAME] = TAG_VALUE;
  temp[GUMBO_TAG_IMG] = TAG_VALUE;
  temp[GUMBO_TAG_INPUT] = TAG_VALUE;
  temp[GUMBO_TAG_ISINDEX] = TAG_VALUE;
  temp[GUMBO_TAG_LI] = TAG_VALUE;
  temp[GUMBO_TAG_LINK] = TAG_VALUE;
  temp[GUMBO_TAG_LISTING] = TAG_VALUE;
  temp[GUMBO_TAG_MARQUEE] = TAG_VALUE;
  ...
  temp[GUMBO_TAG_META] = TAG_VALUE;
  temp[GUMBO_TAG_NAV] = TAG_VALUE;
  temp[GUMBO_TAG_NOEMBED] = TAG_VALUE;
  temp[GUMBO_TAG_NOFRAMES] = TAG_VALUE;
  temp[GUMBO_TAG_NOSCRIPT] = TAG_VALUE;
  temp[GUMBO_TAG_OBJECT] = TAG_VALUE;
  temp[GUMBO_TAG_OL] = TAG_VALUE;
  temp[GUMBO_TAG_P] = TAG_VALUE;
  temp[GUMBO_TAG_PARAM] = TAG_VALUE;
  temp[GUMBO_TAG_PLAINTEXT] = TAG_VALUE;
  temp[GUMBO_TAG_PRE] = TAG_VALUE;
  temp[GUMBO_TAG_SCRIPT] = TAG_VALUE;
  temp[GUMBO_TAG_SECTION] = TAG_VALUE;
  temp[GUMBO_TAG_SELECT] = TAG_VALUE;
  temp[GUMBO_TAG_STYLE] = TAG_VALUE;
  temp[GUMBO_TAG_SUMMARY] = TAG_VALUE;
  temp[GUMBO_TAG_TABLE] = TAG_VALUE;
  temp[GUMBO_TAG_TBODY] = TAG_VALUE;
...
static bool adoption_agency_algorithm(
  ...
  for (unsigned int i = 0; i < 8; ++i) {
    ...
    GumboNode* common_ancestor =
		(GumboNode*)state->_open_elements.data[gumbo_vector_index_of(&state->_open_elements,
                                       formatting_node) -
                                   1];
    ...
    for (int j = 0;;) {
      ...
      // Step 13.9.
      last_node->parse_flags = (GumboParseFlags)(last_node->parse_flags | GUMBO_INSERTION_ADOPTION_AGENCY_MOVED);
    ...
    formatting_node->parse_flags = (GumboParseFlags)(formatting_node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG);
...
static void finish_parsing(GumboParser* parser) {
  ...
  for (GumboNode* node = pop_current_node(parser); node;
    ...
    node->parse_flags = (GumboParseFlags)(node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG);
  }
...
static bool handle_in_head(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
  ...
  } else if (tag_in_02(token, kStartTag)) {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
    ...
    return true;
  } else if (tag_in_04(token, kEndTag)) {
    pop_current_node(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_AFTER_HEAD);
    parser->_parser_state->_reprocess_current_token = true;
    return true;
...
static bool handle_after_head(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
    ...
    return true;
  } else if (tag_in_07(token, kStartTag)) {
    parser_add_parse_error(parser, token);
...
static bool handle_in_body(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    return false;
  } else if (tag_in_08(token, kStartTag) ||
             tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
    return handle_in_head(parser, token);
  } else if (tag_is(token, kStartTag, GUMBO_TAG_BODY)) {
    parser_add_parse_error(parser, token);
    if (state->_open_elements.length < 2 ||
        !node_html_tag_is((GumboNode*)state->_open_elements.data[1], GUMBO_TAG_BODY) ||
    ...
    for (unsigned int i = 0; i < state->_open_elements.length; ++i) {
      if (!node_tag_in_set_09((GumboNode*)state->_open_elements.data[i])) {
        parser_add_parse_error(parser, token);
      }
    }
    ...
    return true;
  } else if (tag_in_09(token, kEndTag)) {
    ...
    bool success = true;
    for (unsigned int i = 0; i < state->_open_elements.length; ++i) {
      if (!node_tag_in_set_0A((GumboNode*)state->_open_elements.data[i])) {
        parser_add_parse_error(parser, token);
        success = false;
        break;
      }
    ...
    return success;
  } else if (tag_in_0A(token, kStartTag)) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in_0B(token, kStartTag)) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    if (node_tag_in_set_0B(
    ...
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in_0C(token, kStartTag)) {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
    ...
    insert_element_from_token(parser, token);
    return result;
  } else if (tag_in_0D(token, kStartTag)) {
    ...
    state->_frameset_ok = false;
    return true;
  } else if (tag_in_0E(token, kEndTag)) {
  ...
  } else if (tag_in_0D(token, kEndTag)) {
  ...
  } else if (tag_in_0B(token, kEndTag)) {
	  GumboTag gumbotag[] = { GUMBO_TAG_H1, GUMBO_TAG_H2, GUMBO_TAG_H3,
    ...
    return success;
  } else if (tag_in_0F(token, kStartTag)) {
    reconstruct_active_formatting_elements(parser);
    add_formatting_element(parser, insert_element_from_token(parser, token));
    return true;
    ...
    return result;
  } else if (tag_in_10(token, kEndTag)) {
    return adoption_agency_algorithm(parser, token, token->v.end_tag);
  } else if (tag_in_11(token, kStartTag)) {
    reconstruct_active_formatting_elements(parser);
    insert_element_from_token(parser, token);
    add_formatting_element(parser, &kActiveFormattingScopeMarker);
    set_frameset_not_ok(parser);
    return true;
  } else if (tag_in_11(token, kEndTag)) {
    GumboTag token_tag = token->v.end_tag;
    ...
    return true;
  } else if (tag_in_12(token, kStartTag)) {
    bool success = true;
    ...
    if (tag_is(token, kStartTag, GUMBO_TAG_IMAGE)) {
      ...
      node->parse_flags = (GumboParseFlags)(node->parse_flags | GUMBO_INSERTION_FROM_IMAGE);
    }
    pop_current_node(parser);
    acknowledge_self_closing_tag(parser);
    set_frameset_not_ok(parser);
    ...
    acknowledge_self_closing_tag(parser);
    return true;
  } else if (tag_in_13(token, kStartTag)) {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
    acknowledge_self_closing_tag(parser);
    return true;
    ...
    return true;
  } else if (tag_in_14(token, kStartTag)) {
    ...
    insert_element_from_token(parser, token);
    return true;
  } else if (tag_in_15(token, kStartTag)) {
    bool success = true;
    GumboTag exception =
		tag_in_16(token, kStartTag)
            ? GUMBO_TAG_RTC
            : GUMBO_TAG_LAST;
    ...
    return true;
  } else if (tag_in_17(token, kStartTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
...
static bool handle_in_table(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_CHARACTER ||
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_COLUMN_GROUP);
    return true;
  } else if (tag_in_18(token, kStartTag)) {
    clear_stack_to_table_context(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE_BODY);
    if (tag_in_19(token, kStartTag)) {
      insert_element_of_tag_type(
    ...
    return true;
  } else if (tag_in_1A(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else if (tag_in_1B(token, kStartTag) ||
             (tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE))) {
    return handle_in_head(parser, token);
  } else if (tag_is(token, kStartTag, GUMBO_TAG_INPUT) &&
             attribute_matches(
...
static bool handle_in_caption(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kEndTag, GUMBO_TAG_CAPTION)) {
  ...
  } else if (tag_in_1C(token, kStartTag) ||
    ...
    parser->_parser_state->_reprocess_current_token = true;
    return true;
  } else if (tag_in_1D(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_table_body(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kStartTag, GUMBO_TAG_TR)) {
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_ROW);
    return true;
  } else if (tag_in_1E(token, kStartTag)) {
    parser_add_parse_error(parser, token);
    clear_stack_to_table_body_context(parser);
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_ROW);
    return false;
  } else if (tag_in_1F(token, kEndTag)) {
    ...
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_TABLE);
    return true;
  } else if (tag_in_20(token, kStartTag) ||
             tag_is(token, kEndTag, GUMBO_TAG_TABLE)) {
    ...
    parser->_parser_state->_reprocess_current_token = true;
    return true;
  } else if (tag_in_21(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_row(GumboParser* parser, GumboToken* token) {
  ...
  } else if (tag_in_22(token, kStartTag) ||
             tag_is(token, kEndTag, GUMBO_TAG_TABLE)) {
    if (!has_an_element_in_table_scope(parser, GUMBO_TAG_TR)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
  ...
  } else if (tag_in_23(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else {
...
static bool handle_in_cell(GumboParser* parser, GumboToken* token) {
  ...
  } else if (tag_in_24(token, kStartTag)) {
  ...
  } else if (tag_in_25(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  } else if (tag_in_26(token, kEndTag)) {
    if (!has_an_element_in_table_scope(parser, token->v.end_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
...
static bool handle_in_select(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    GumboVector* open_elements = &parser->_parser_state->_open_elements;
    if (node_html_tag_is(get_current_node(parser), GUMBO_TAG_OPTION) &&
        node_html_tag_is((GumboNode*)open_elements->data[open_elements->length - 2],
            GUMBO_TAG_OPTGROUP)) {
      pop_current_node(parser);
    }
    if (node_html_tag_is(get_current_node(parser), GUMBO_TAG_OPTGROUP)) {
    ...
    return false;
  } else if (tag_in_27(token, kStartTag)) {
    parser_add_parse_error(parser, token);
    if (!has_an_element_in_select_scope(parser, GUMBO_TAG_SELECT)) {
      ignore_token(parser);
    } else {
    ...
    return false;
  } else if (tag_in_28(token, kStartTag) ||
             tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE)) {
    return handle_in_head(parser, token);
  } else if (token->type == GUMBO_TOKEN_EOF) {
    return handle_in_body(parser, token);
...
static bool handle_in_select_in_table(GumboParser* parser, GumboToken* token) {
  if (tag_in_29(token, kStartTag)) {
    parser_add_parse_error(parser, token);
    close_current_select(parser);
    parser->_parser_state->_reprocess_current_token = true;
    return false;
  } else if (tag_in_29(token, kEndTag)) {
    parser_add_parse_error(parser, token);
    if (!has_an_element_in_table_scope(parser, token->v.end_tag)) {
      ignore_token(parser);
      return false;
